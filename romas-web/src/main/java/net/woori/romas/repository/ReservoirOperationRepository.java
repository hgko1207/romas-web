...
public interface ReservoirOperationRepository extends DefaultRepository<ReservoirOperation, CompositeOperationPK>  {
	...
	@Query(value = "SELECT facility_name FROM tb_reservoir_operation WHERE branch = ?1 GROUP BY facility_name", nativeQuery = true)
	List<String> getFacilityNameList(String branch);
